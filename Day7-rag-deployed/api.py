...
host = os.getenv("ORACLE_HOST")          # e.g.: adb.us-ashburn-1.oraclecloud.com
port = os.getenv("ORACLE_PORT", "1522")
service_name = os.getenv("ORACLE_SERVICE_NAME")  # e.g.: xxxxx_high.adb.oraclecloud.com

if not all([password, host, service_name]):
    raise ValueError("Oracle connection info missing in environment variables!")

# EZ Connect string (wallet not required)
dsn = f"{username}/{password}@(description=(retry_count=20)(retry_delay=3)" \
